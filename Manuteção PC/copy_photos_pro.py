...
def select_files(file_info, num_photos):
    ...
    for _ in range(random.randint(1, 10)):  # Embaralha a lista 1 a 10 vezes
...
file_info = [(f, os.path.getatime(os.path.join(source_dir, f))) for f in os.listdir(source_dir) if os.path.isfile(os.path.join(source_dir, f)) and filetype.is_image(os.path.join(source_dir, f))]
...
# Se o número de arquivos selecionados for menor que o número desejado, selecione arquivos adicionais aleatoriamente
if len(selected_files) < num_photos:
    additional_files_needed = num_photos - len(selected_files)
    remaining_files = [f for f, _ in file_info if f not in selected_files and f not in last_selected]
    if len(remaining_files) < additional_files_needed:
        remaining_files = [f for f, _ in file_info if f not in selected_files and f not in last_selected]
    additional_files = random.sample(remaining_files, additional_files_needed)
...
for file in tqdm(selected_files):
    ...
    # Verifique se o nome randômico já existe no diretório de destino. Se sim, gere um novo nome.
    while os.path.exists(os.path.join(destination_dir, random_name)):
        random_name = ''.join(random.choices(string.ascii_letters + string.digits, k=RANDOM_NAME_LENGTH)) + file_extension
